...
QuestionObj.prototype.init =  function(opts){
	...
	// 插入
	this.insertQuestion(opts);
	//上移下移
	if(opts.animate){
		this.moveMehod(opts);
	}
	this.addoneQuestion(opts);
	//单选
	if(opts.animate){
...
QuestionObj.prototype.sortIndex = function(){
		...
		section.each(function(i,name){
			if(i == section.length - 1){
				// $(this).remove();
				console.log("去掉的==", i, name)
				return false;
			}
			console.log("", i,name)
			// $(this).remove("id");
			$(name).attr("id","item_"+i+"");
			$(name).find(".number").text(i+1);

			
			if(i==0){
				$('#item_0').find('.btn_add2').hide()
			}else{
				$('#item_'+i+'').find('.btn_add2').show()
			}
		});
		//执行隐藏第一个的插入框		  
		// $('#item_0').find('.btn_add2').hide()
...
//默认增加一道题
QuestionObj.prototype.addoneQuestion = function (opts) {	
	$('#item_0').find('.btn_add2').hide()
};

QuestionObj.prototype.addQuestion = function(opts){
	$(".footer").undelegate("click").delegate("#addQuestion", "click",function(){
		if(opts.sectionNum == (opts.addAlmostNum - 1)){
			if (Message) {
				Message.error("最多只能添加"+opts.addAlmostNum+"道题");
			}
			return;
		}
		//sectionNum当前判定后进行重置
		...
		QuestionObj.insertQuestion(opts)
	})
...
//插入
QuestionObj.prototype.insertQuestion=function (opts) {
		let insert = document.getElementsByClassName("btn_add2");
		for (let i = 0; i < insert.length; i++) {
			insert[i].onclick=function () {
			    ...
			    opts.sectionNum = $(".content").find("section").length - 1;
			    opts.sectionNum++;
			    $(this).parent().before(QuestionObj.createProblem(opts.sectionNum, opts));
			    if (opts.callback != undefined) {
			      opts.callback(QuestionObj.createProblem(opts.sectionNum, opts), "item_" + opts.sectionNum + "");
			    }
			    QuestionObj.addOptions(opts);
