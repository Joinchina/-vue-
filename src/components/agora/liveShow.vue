<template>
  <div id="liveWarp">
    ...
    <div v-if="hostFlag"
      ...
      <el-button v-if="!haveClass"
                 type="primary"
                 class="cursorOpera"
                 @click="publish">开始上课</el-button>
      <el-button v-if="haveClass"
                 type="danger"
                 class="cursorOpera"
                 @click="unpublish">下课</el-button>
    </div>
    <el-row type="flex"
            class="row-bg"
      ...
      <el-col :span="4">
        <div class="grid-content bg-purple">
          <div v-for="(item,index) in originStream"
               :class="{liveItem:true,bigScreen:item.bigScreen}"
               :key="index"
               :id="item.playAreaId+item.getId()"
               @dblclick.stop="scanle">
            <div v-if="item.liveHost"
                 class="suspensionLayer">
              <span :class="{operaIcon:true, closedVedio:closeVedio,openVedio:!closeVedio}"
                    @click.stop="closeVedioHandle" />
              <span :class="{operaIcon:true, closedAudio:closeAudio,openAudio:!closeAudio}"
                    @click.stop="closeAudioHandle" />
            </div>
          </div>
        </div>
      </el-col>
      <el-col :span="15">
        ...
        <div class="grid-content bg-purple-light liveHostBox">
          <div class="middleTop">
            <div class="rightText">
              <div v-if="!hostFlag">
                <img class="volummIcon"
                     src="../../assets/images/liveIcon/sound48.png"
                     alt="">
                <el-slider v-model="volumeValue"
                           @change="changeHostVolume" />
                <el-button :type="attachment?'danger':'primary'"
                           size="mini"
                           round
                           @click.stop="attachmentHandle">{{ attachmentText }}</el-button>
              </div>
              <div v-if="hostFlag">
                <span>摄像头：</span>
                ...
                <el-button type="primary"
                           size="mini"
                           round
                           @click.stop="cameraVisible=true">切换</el-button>
              </div>
            </div>
          </div>

          <div v-for="(item,index) in hostStream"
               v-show="hostShowFlag"
               :class="{liveItem:true,bigScreen:item.bigScreen}"
               :key="index"
               :id="item.playAreaId+item.getId()"
      ...
      <el-col :span="4">
        <div class="grid-content bg-purple">
          ...
          <ul class="chat">
            <li v-for="(item,index) in chatArray"
                :key="index"
                class="clearfix">
          ...
          <div class="messageInner">
            ...
            <el-button type="info"
                       size="mini"
                       @click="sendChannelMsg">发送<i class="el-icon-message el-icon--right" /></el-button>

            <el-button type="info"
                       size="mini"
                       @click="sendPtoPMsg">点对点<i class="el-icon-message el-icon--right" /></el-button>

            <el-button type="info"
                       size="mini"
                       @click="invitatePerson">连线贾君彦<i class="el-icon-message el-icon--right" /></el-button>
    ...
    <el-dialog :visible.sync="cameraVisible"
               ...
               width="30%"
               @open="getCamera">
      <el-select v-model="usedCameraVal"
                 placeholder="请选择">
        <el-option v-for="item in cameraArr"
                   :key="item.deviceId"
                   :label="item.label"
                   :value="item.deviceId" />
      </el-select>
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="switchCamera">确 定</el-button>
      </span>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    joinRoom () {
      ...
      _this.hostFlag = _this.role != 'STUDENT'
    ...
    attachmentHandle () {
      const _this = this
      const closeattachment = this.attachment
...
.liveItem {
  ...
  .suspensionLayer {
    ...
    overflow: hidden;
    .closedVedio {
      background: url(../../assets/images/liveIcon/close_video_default.png)
        no-repeat center 6px;
    }
    .openVedio {
      background: url(../../assets/images/liveIcon/open_video_default.png)
        no-repeat center 6px;
    }
    .closedAudio {
      background: url(../../assets/images/liveIcon/icon_voice-8151f7d0.png)
        no-repeat center 6px;
    }
    .openAudio {
      background: url(../../assets/images/liveIcon/microphone_default.png)
        no-repeat center 6px;
    }
    .operaIcon {
