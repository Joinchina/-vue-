	...
	export default {
		...
		methods: {
			...
			getTaskOne() {
				...
				Integral.getTaskOne(data, (res => {
					console.log("单个积分", res);
					if(res.status == 200) {
		    ...
		    editIntegral() {
		    	...
		    	this.loading = true;
		    	Integral.postTaskUpdate(data, (res => {
		    		console.log("编辑积分成功", res)
		    		if(res.status == 200) {
						this.close();
						this.$emit('updateList');
						this.$message.success("编辑积分成功！");
						this.loading = false;
					} else {
						this.$message.error(res.message);
						this.loading = false;
