...
<script>
	...
	export default {
		...
		props: ['dialogEditProfessor', 'orgId', 'role', 'userId'],
		created(){
		...
		methods: {
			open(){
				...
				this.$nextTick(() => {
		          	...
		          	if(this.editorProfessor2.id) {
			            this.editorProfessor2.txt.html("");
			        } else {
			        	this.wangEditp2();
			        }
			...
			wangEditp2() {
	        let editorProfessor2 = new E('#edit_menup2', '#edit_contentp2')
	            
	            editorProfessor2.customConfig.menus = [
	                'head',  // 标题
	                'bold',  // 粗体
	                'fontSize',  // 字号
	                'foreColor',  // 文字颜色
	            ...
	            // 配置服务器端地址
	            editorProfessor2.customConfig.customUploadImg = (files, insert) => {
	                // aliyun 上传图片
	            ...
	            editorProfessor2.customConfig.onchange = (html) => {
	                this.ruleEditProfessor.description = html
	            };
	            editorProfessor2.customConfig.zIndex = 1;
	            editorProfessor2.create();
	            this.editorProfessor2 = editorProfessor2;
			...
			getUser () {
				...
				model.getUser(data, ( res => {
					...
					if( res.status == 200) {
						...
						this.editorProfessor2.txt.html(this.ruleEditProfessor.description);
					} else {
						this.$message.error(res.message);
					}
				}));
			...
			close () {
				...
				this.resetForm('ruleEditProfessor');
				this.ruleEditProfessor.photo = "";
				this.imageUrl = "";
			...
			editProfessor () {
				...
				model.orgEditProfessor(data, ( res => {
					...
					if( res.status == 200) {
						...
						bus.$emit("updateProfessorUserList", "PROFESSOR")
