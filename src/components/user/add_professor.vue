<template>
	<div id="waddProfessor">
		<el-dialog class="dialog-form w-wang" title="新增专家"
			:visible.sync = "dialogAddProfessor"
			:close-on-click-modal="false"
			:before-close="close"
			@open="open">
			<el-form :model="ruleAddProfessor" :rules="rules" ref="ruleAddProfessor"
					v-loading="loading"
				...
				<el-form-item label="专家简介：" :label-width="labelWidth" prop="workPlace">
				    <!-- <el-input type="textarea" v-model.trim="ruleAddProfessor.description"></el-input> -->
...
<script>
	...
	export default {
		...
		props: ['dialogAddProfessor', 'orgId', 'role'],
		created(){
		...
		methods: {
			open() {
				...
				this.$nextTick(() => {
		          	...
		          	if(this.editorProfessor1.id) {
			            this.editorProfessor1.txt.html("");
			        } else {
			        	this.wangEditp1();
			        }
		        })
			...
			wangEditp1() {
	        let editorProfessor1 = new E('#edit_menup1', '#edit_contentp1')
	            
	            editorProfessor1.customConfig.menus = [
	                'head',  // 标题
	                'bold',  // 粗体
	                'fontSize',  // 字号
	                'foreColor',  // 文字颜色
	            ...
	            // 配置服务器端地址
	            editorProfessor1.customConfig.customUploadImg = (files, insert) => {
	                // aliyun 上传图片
	            ...
	            editorProfessor1.customConfig.onchange = (html) => {
	                this.ruleAddProfessor.description = html
	            };
	            editorProfessor1.customConfig.zIndex = 1;
	            editorProfessor1.create();
	            this.editorProfessor1 = editorProfessor1;

		    },
			close () {
				this.$emit('update:dialogAddProfessor', false);
				this.resetForm('ruleAddProfessor');
			},
			...
			addProfessor () {
				...
				this.loading = true;
				model.orgAddProfessor(data, ( res => {
					// console.log("addProfessor", res);
					if( res.status == 200) {
