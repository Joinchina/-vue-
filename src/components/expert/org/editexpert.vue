<template>
    <div class="addeditbox">
        ...
        >  
            ...
            <div class="form_one" v-show="formone">
                <el-form :model="form" :rules="rules_one" ref="form">                                                                      
                    ...
                    <el-form-item label="任教学段：" :label-width="formLabelWidth" prop="studySectionId">
                        <template>
                                <el-select v-model="form.studySectionId" placeholder="请选择学段" @change="setstudent">
                                    <el-option
                    ...
                    <el-form-item label="专家照片：" :label-width="formLabelWidth"  class="avatar-uploader">
                            ...
                            <label for="fileName" class="sel2"  style="margin-left:0px;margin-top:4px">添加照片</label>
                            <input class="sel1 select-file"  style="margin-top:4px" multiple="true" id="fileName" type="file" accept="image/png,image/gif,image/jpeg"  @change="selAvatar($event)">
                            <div style="margin-top:44px;margin-left:40px;">
                                <ul v-if="!!form.avatar">
                                    <li >
                                        <img :src="form.avatar" class="avatar" style="width:150px;height: 150px;">
                ...
                <button class="nextdown" @click="nextdown">下一步</button>  
            </div>
            <div class="form_two" v-show="formtwo">
                <div class="clazzbox" v-for="(item,index) in form.expertCourses" :key="index">
                    <div class="clazz_top">
                        <h1>课程{{index+1}}：</h1>
                        <img src="../../../assets/img/expert/xxx.png" alt="" @click="deleteArr(index)"  v-show="index!=0 || form.expertCourses.length>1">
                    </div>
...
export default {
   data () {
       ...
       return {
           //任教学段
            ...
            formtwo:false,
            isedit:false,//是否是编辑
            citycode:'',//编辑时传递给子组件的城市id
       }
   },
   props: ['editexpertshow','expertinfo'],
   watch:{
     expertinfo:'editor'  
   },
   created(){
   ...
   methods:{
       ...
       editor(){
           ...
           if(!this.expertinfo.curriculumFieldIds){
               this.$set(this.expertinfo,'curriculumFieldIds',[]);
           };
           
           this.form=this.expertinfo
           this.citycode=this.form.areaCode
           //编辑时对应上学科
           this.setstudent()
           //数据传递给富文本编辑器
           this.descont=this.form.description;                              
       },
      //富文本编辑器获取的信息
      updatEditor(val){
      ...
      selAvatar(event){
                        ...
                        async function resumeUpload(myFile) {
                            for (let i = 0; i < 5; i++) {
                                try {
                                    ...
                                    _this.$set(_this.form,'avatar',result.res.requestUrls[0].split('?').shift())
                                    console.log(result);
        ...
        //关闭模态框
        close () {
          this.$emit('update:editexpertshow', false);
      ...
      //点击删除评价信息
      deleteArr(index){
          this.form.expertCourses.splice(index,1)
      },
      //点击添加信息
      addclazzinfo(){
          let newobg={
         ...
         this.form.expertCourses.push(newobg)   
      },
      //新增专家信息
      addfield(){
          if(!this.form.name){
