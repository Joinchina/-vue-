<template>
  <div class="j-view">
    <el-dialog class="dialog-form pointsItemdialog" title="查看积分"
               :visible.sync="dialogViewPoints"
      ...
      <div class="tableIntegralBox">
        ...
        <el-table class="list"
          ...
          <el-table-column
            ...
            label="分值">
            <template slot-scope="scope">
              <p>{{ scope.row.integral | filterIntegral}}</p>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
  ...
  export default {
    ...
    created(){
      var _this = this;
      bus.$on("itergralView",function (value) {
        _this.userIdOn  = value;
        var data = {
          userId:_this.userIdOn
        }
        _this.viewIntergral(data);
      })
    },
    beforeDestroy() {
    ...
    methods: {
      viewIntergral(data){
        model.IntegrafindByUserId(data, ( res => {
          if(res.status == 200) {
      ...
      exportIntegralItem(){
        ...
        window.open(API.urls.exportIntegralAll+"?token="+getToken()+"&userId="+this.userId)
      },
      close () {
        this.$emit('update:dialogViewPoints', false);
        this.miniAvatar="";
      }
    }
  }
