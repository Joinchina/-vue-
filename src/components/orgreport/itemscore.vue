<template>
	<div class="w-itemscore">

		<section class="handle clearfix">
			<span>
				<Goback passtitle="项目评分" ></Goback>
	...
	export default {
		...
		methods: {
			...
			getOrgData() {
				...
				orgReport.RepOrgItemGrade(data, (res => {
					console.log('机构-项目评分', res);
					if(res.status == 200) {
						...
						this.content.forEach((val, ind, arr) => {
							let content2 = val.questionPaperStatistics;
			...
			getItemData() {
				...
				orgReport.RepItemItemGrade(data, (res => {
					console.log('项目-项目评分', res);
					if(res.status == 200) {
						this.totalElements = res.data.totalElements;
						this.loading_font=this.$globalParam.dataEmpty;
						this.loading = false;
			...
			resExport() {
				...
				if(this.role == 'PROJECTADMIN') {
					window.open(API.urls.RepItemExpItemGrade+"?orgId="+this.orgId+"&token="+getToken());
				} else {
					window.open(API.urls.RepOrgExpItemCrade+"?orgId="+this.orgId+"&token="+getToken());
				}
