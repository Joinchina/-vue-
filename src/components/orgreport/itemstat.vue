<template>
	<div class="w-itemstat">

		<section class="handle clearfix">
			<span>
				<Goback passtitle="项目统计"></Goback>
	...
	export default {
		...
		methods: {
			...
			getOrgData() {
				...
				orgReport.RepOrgItemStat(data, (res => {
					console.log('机构-项目统计', res);
					if(res.status == 200) {
			...
			getItemData() {
				...
				orgReport.RepItemItemStat(data, (res => {
					console.log('项目-项目统计', res);
					if(res.status == 200) {
						this.items = res.data.content;
						this.totalElements = res.data.totalElements;
	        ...
	        resExport() {
				...
				if(this.role == 'PROJECTADMIN') {
					window.open(API.urls.RepItemExpItemStat+"?orgId="+this.orgId+"&token="+getToken());
				} else {
					window.open(API.urls.RepOrgExpItemStat+"?orgId="+this.orgId+"&token="+getToken());
				}
