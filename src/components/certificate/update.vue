<template>
	...
	<Goback passtitle="证书设置" path="/org/manage" :query="query"></Goback>
	<!-- 保存证书 -->
	<div class="handle2">
		...
		<el-form class="form-box" 
			...
			<el-form-item label="证书背景：" prop="bgUrl">
				...
				<div class="cert-img-box" v-if="form.bgUrl">
					<i class="icon-del-circle" @click="delImg('bgUrl')"></i>
			...
			<el-form-item label="证书右章：" prop="rightSignetUrl">
				...
				<div class="cert-img-box" v-if="form.rightSignetUrl">
					<i class="icon-del-circle" @click="delImg('rightSignetUrl')"></i>
			...
			<el-form-item label="证书左章：" prop="leftSignetUrl">
				...
				<div class="cert-img-box" v-if="form.leftSignetUrl">
					<i class="icon-del-circle" @click="delImg('leftSignetUrl')"></i>
			...
			<div class="cert-pop-box ">
				...
				<div class="cert-bottom clearfix">
					<div class="leftbox">
						...
						<img class="left-signet" v-if="form.leftSignetUrl" :src="form.leftSignetUrl" alt="">
					</div>

					<div class="rightbox">
						<p class="right-name">{{form.rightName}}</p>
						<img class="right-signet" v-if="form.rightSignetUrl" :src="form.rightSignetUrl" alt="">
					</div>
				</div>
			</div>	
		</div>
	...
	methods: {
		...
		getCertificate() {
			...
			certificate.getOrgCert(data, (res => {
				...
				if(res.status == 200) {
					this.form.bgUrl = res.data.backgroundUrl;
					this.form.rightSignetUrl = res.data.rightStampUrl;
					this.form.rightName = res.data.rightStampName;
					this.form.leftSignetUrl = res.data.leftStampUrl;
					this.form.leftName = res.data.leftStampName;
		...
		submitForm(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					...
					console.log("size ", this.form.bg, sizebg, sizer, sizel);
					if(sizebg > 10485760) {
		...
		certificateUpdate() {
			let data = {
				...
				delete: this.deleteLeftSignet
			}
			console.log("编辑参数", data);

			certificate.orgCertUpdate(data, (res => {
				// console.log("编辑证书", res);
				if(res.status == 200) {
		...
		delImg(str) {
			...
			switch(str) {
				case 'bgUrl':
					...
					break;
				case 'rightSignetUrl':
					this.form.rightSignetUrl = "";
					this.form.rightSignet = {};
					break;
				case 'leftSignetUrl':
					this.form.leftSignetUrl = "";
					this.form.leftSignet = {};
					this.deleteLeftSignet = true;
					break;
			}
		}
	...
	.form-box {
		...
		.cert-img-box {
			...
			> i {
				...
				cursor: pointer;
				&:hover {
					background-image: url('../../assets/img/icon/icon-del-circle-blue.png');
				}
			}
			>img {
				width: 140px;
